...
const ProductCard = ({ productDetails }) => {
  ...
  return (
    <div className="p-4 flex flex-col justify-between bg-white shadow-md rounded-lg">
      ...
      <img src={images[0]} alt={name} className="w-full  object-cover rounded-t-lg" />
      <div className=" flex flex-col gap-2">
        <h3 className="text-lg font-semibold">{name}</h3>
        <p className="text-gray-500">{tagLine}</p>
        <Link to={`EV/${productDetails._id}`} className=" w-full px-4 py-2 bg-black text-white text-center rounded-md hover:bg-slate-800 focus:outline-none">
          View Details
        </Link>
      </div>
    </div>
